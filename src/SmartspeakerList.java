...
public class SmartspeakerList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/* Checks the Smartspeaker type whether it is electronicArts or activision or takeTwoInteractive */
		...
		{
			if(CategoryName.equals("smartspeaker"))
			{
				for(Map.Entry<String,Smartspeaker> entry : SaxParserDataStore.smartspeakers.entrySet())
				{
					if(entry.getValue().getRetailer().equals("Smartspeaker"))
					{
			...
			else if(CategoryName.equals("voiceassistant"))
			{
				for(Map.Entry<String,Smartspeaker> entry : SaxParserDataStore.smartspeakers.entrySet())
				{
					if(entry.getValue().getRetailer().equals("PrevAccessory"))
					{
						hm.put(entry.getValue().getId(),entry.getValue());
					}
				}
		...
		for(Map.Entry<String, Smartspeaker> entry : hm.entrySet()){
			...
			pw.print("<li id='item'><img src='images/smartspeaker/"+smartspeaker.getImage()+"' alt='' /></li>");
